CREATE OR REPLACE FUNCTION alter_admin_foreign_servers(host_name varchar, data_warehouse_db_name varchar, ff_new_db_name varchar, postgres_password varchar) --Admin
RETURNS void AS $$
DECLARE
...
BEGIN
  ...
  EXECUTE 'alter server foreign_data_warehouse options (set dbname ''' || data_warehouse_db_name || ''')';
  ...
  EXECUTE 'alter server foreign_ff_new options (set dbname ''' || ff_new_db_name || ''')';
