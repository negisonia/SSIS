...
--VALIDATE RPT DRUG
SELECT  array_to_json(array_agg(row_to_json(t))) from (SELECT q.indication_name, q.indication_abbre, q.benefit_name, q.criteria_restriction_name, q.criteria_restriction_short_name, q.market_name, q.drug_name, q.lives, q.health_plan_count, q.provider_count, q.total_pharmacy_lives, q.total_medical_lives, q.total_health_plan_count, q.total_provider_count  from (SELECT * FROM rpt_geo(_report_id, _benefit_type, _result_market_id, _dim_criteria_restriction_id, _drug_id, _market_type_id, _market_id) order by dim_criteria_restriction_id, drug_id) q) t INTO rpt_geo_output;
