CREATE OR REPLACE FUNCTION common_create_health_plan_type(is_active BOOLEAN, type_name VARCHAR, is_commercial BOOLEAN, is_medicare BOOLEAN) --FF_NEW DB
RETURNS INTEGER AS $$
DECLARE
...
IF health_plan_type_id IS NULL THEN		
	...
	VALUES ( CASE when is_active IS NULL THEN TRUE ELSE is_active END, type_name, type_name, NULL, CASE when is_commercial IS NULL THEN FALSE ELSE is_commercial END, CASE when is_medicare IS NULL THEN FALSE ELSE is_medicare END, NULL, NULL) RETURNING id INTO health_plan_type_id;
