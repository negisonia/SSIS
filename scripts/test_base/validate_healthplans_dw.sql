CREATE OR REPLACE FUNCTION validate_healthplans_data()-- DATA WAREHOUSE
RETURNS BOOLEAN AS $$
DECLARE
...
IF FF_HEALTH_PLANS_COUNT = FE_HEALTH_PLANS_COUNT THEN
	...
	IF FF_HEALTH_PLANS_COUNT = FF_FE_MERGED_HEALTH_PLANS_COUNT THEN
		SUCCESS :=TRUE;
	ELSE
		select throw_error('FF AND FE SCHEMAS CONTAINS DIFFERENT HEALTHPLANS');
		SUCCESS:=FALSE; 	
	END IF;
ELSE
	select throw_error('FF AND FE SCHEMAS CONTAINS DIFFERENT HEALTHPLANS');
...
IF FF_HEALTH_PLANS_COUNT = DW_FF_HEALTH_PLANS_COUNT THEN
	...
	IF FF_HEALTH_PLANS_COUNT = FF_DW_MERGED_HEALTH_PLANS_COUNT THEN
		SUCCESS :=TRUE;
	ELSE
		select throw_error('FF AND DW SCHEMAS CONTAINS DIFFERENT HEALTHPLANS');
		SUCCESS:=FALSE; 	
	END IF;
ELSE
	select throw_error('FF AND DW SCHEMAS CONTAINS DIFFERENT HEALTHPLANS');
