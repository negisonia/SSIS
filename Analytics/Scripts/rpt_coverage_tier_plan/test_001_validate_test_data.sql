...
-- Validate that each plan has a Drug 003 associated
FOREACH plan_name IN ARRAY health_plan_names
  LOOP

    SELECT calculate_report_value_varchar('drug_name', get_report_name_call('rpt_coverage_tier_plan', ARRAY[criteria_report_id,current_month_int]),'health_plan_name=''' || plan_name || '''') INTO actual_value;
    expected_value = 'DRUG_003';

    PERFORM validate_comparison_values_varchar(actual_value, expected_value,'ana_rpt_coverage_tier_plan_test_001_validate_data-error: EXPECTED drug_name TO BE ');
