CREATE OR REPLACE FUNCTION res_general_test_004_validate_test_data() --DATA WAREHOUSE
RETURNS boolean AS $$
DECLARE
...
LOOP
		...
		IF textValue = 'drug_2' THEN
			...
			IF intValue IS NULL THEN
				SELECT throw_error('restrictions_test_004_validate_test_data ERROR: MISSING DRUG '|| textValue);
			ELSE
				--VERIFY DRUG JCODE
				...
				IF intValue2 IS NULL THEN
					SELECT throw_error('restrictions_test_004 MISSING JCODE DRUG ASSOCIATION');
				END IF;
			END IF;
		...
		ELSIF textValue = 'drug_6' THEN
			...
			ELSE
				...
				IF intValue2 IS NULL THEN
					SELECT throw_error('restrictions_test_004_validate_test_data ERROR: MISSING JCODE DRUG ASSOCIATION');
				END IF;
			END IF;
		--VALIDATE DRUG
		ELSE
			IF textValue = 'drug_10_inactive' THEN
				...
				IF intValue IS NOT NULL THEN
					SELECT throw_error('restrictions_test_004_validate_test_data ERROR: DRUG  SHOULD NOT EXISTS'|| textValue);
				END IF;
