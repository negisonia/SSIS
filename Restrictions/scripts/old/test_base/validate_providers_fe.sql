CREATE OR REPLACE FUNCTION validate_providers_data()-- FRONT END
RETURNS BOOLEAN AS $$
DECLARE
...
IF FF_PROVIDERS_COUNT = FE_PROVIDERS_COUNT THEN
	...
	IF FF_PROVIDERS_COUNT = FF_FE_MERGED_PROVIDER_COUNT THEN
		SUCCESS :=TRUE;
	ELSE
		select throw_error('FF SOURCE DATA AND FRONT END MATERIALIZED VIEW CONTAINS DIFFERENT PROVIDERS');	
	END IF;
ELSE
	select throw_error('FF SOURCE DATA AND FRONT END MATERIALIZED VIEW CONTAINS DIFFERENT PROVIDERS');
