...
CASE textValue 
	WHEN 'restrictions_test_hp_commercial_1' THEN
		...
		ELSE
			...
			SELECT EXISTS (SELECT 1 FROM health_plan_types hpt WHERE hpt.id=intValue and hpt.is_active IS TRUE) INTO booleanValue;
			IF booleanValue IS FALSE THEN
	...
	WHEN 'restrictions_test_hp_commercial_3' THEN	
		--VALIDATE HEALTH PLAN EXISTS AND IS ACTIVE
		SELECT EXISTS (SELECT 1 FROM health_plans h WHERE h.name=textValue and h.is_active IS TRUE) INTO booleanValue;
		IF booleanValue IS FALSE THEN
			select throw_error('test_001_validate_test_data-error: EXPECTED HEALTH PLAN ' || textValue || ' DOES NOT EXISTS OR IS INACTIVE');
