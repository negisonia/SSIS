CREATE OR REPLACE FUNCTION createReportCriteriaGroups(reportId integer,reportClientiId integer, restrictionIds integer[], groupName varchar)
RETURNS boolean AS $$
DECLARE
		...
		ELSE
			...
			LOOP
				...
				IF restictionBelongsToReport = false THEN
				        RAISE NOTICE 'RESTRICTION ID DOES NOT BELONG TO THE REPORT';	
					success:=false; 
					RETURN success;	
				END IF;				
			END LOOP;
